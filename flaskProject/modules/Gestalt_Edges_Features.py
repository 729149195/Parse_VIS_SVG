...
class GestaltSimilarityCalculator:
    ...
    def calculate_tag_similarity(self, node_attrs, other_node_attrs):
        ...
        if node_attrs.get('tag').split('_')[0] == other_node_attrs.get('tag').split('_')[0]:
            tag_match = 1.0
        elif node_attrs.get('tag').split('_')[0] != other_node_attrs.get('tag').split('_')[0]:
    ...
    def calculate_position_similarity(self, node_attrs, other_node_attrs):
        ...
        max_difference = 30  # 可以调整，以适应不同的场景
    ...
    def calculate_gestalt_matrix(self, node_attrs, other_node_attrs):
        ...
        a5 = 1.0
        a6 = 1.0
        color_similarity['fill_color_difference'] *= a1
        color_similarity['opacity_difference'] *= a2
        color_similarity['brightness_difference'] *= a3
        color_similarity['saturation_difference'] *= a4
        color_similarity['stroke_color_similarity'] *= a5
        color_similarity['stroke_width_similarity'] *= a6
        a7 = 1.0
        a8 = 1.0
        a9 = 0.0
        tag_similarity['tag_match'] *= a7
        tag_similarity['layer_similarity'] *= a8
        tag_similarity['text_content_similarity'] *= a9
        a10 = 1.0
        a11 = 1.0
        a12 = 1.0
        a13 = 1.0
        a14 = 1.0
        a15 = 1.0
        position_similarity['top_edge_similarity'] *= a10
        position_similarity['bottom_edge_similarity'] *= a11
        position_similarity['left_edge_similarity'] *= a12
        position_similarity['right_edge_similarity'] *= a13
        position_similarity['area_similarity'] *= a14
        position_similarity['overlap_ratio'] *= a15
        weight = 0
        for key in color_similarity:
            weight += color_similarity[key] / 13
        for key in tag_similarity:
            weight += tag_similarity[key] / 13
...
def update_graph_with_similarity_edges():
    ...
    most_common_value, most_common_count = values_count.most_common(1)[0]  # 找到众数

    # 过滤小于众数的边，同时排除权重为0的边
    filtered_gestalt_edges = [edge for edge in gestalt_edges if edge[2] >= most_common_value and edge[2] > 0]
